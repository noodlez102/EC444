...
In order to use I2C to communicate with the accelerometer sensor, we first need to configure master mode on ESP32. Then, we use the i2c_scanner function to detect the address of the accelerometer sensor. There are two different I2C helper functions: writeRegister, readRegister, and read16. The writeRegister function writes a byte to a register on the sensor. The readRegister function reads a byte from a register on the sensor. The read16 function reads 16 bits (2 bytes) from the sensor. To get acceleration, we use the getAccel function to read the x, y, and z values from the sensor. We then use the calcRP function to calculate the roll and pitch values from the x, y, and z values. Finally, we create a task to continuously poll the accelerometer sensor and calculate the roll and pitch values. The values are printed to the console every 500ms.
