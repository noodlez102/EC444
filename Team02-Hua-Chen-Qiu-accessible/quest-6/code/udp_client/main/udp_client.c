        ...
        while (1) {
            ...
            // Data received
                ...
                if (strncmp(rx_buffer, "1",1) == 0) {
                    gesture = 1; // Rock
                    ESP_LOGI(TAG, "Gesture set to Rock");
                } else if (strncmp(rx_buffer, "2",1) == 0) {
                    gesture = 2; // Paper
                    ESP_LOGI(TAG, "Gesture set to Paper");
                } else if (strncmp(rx_buffer, "3",1) == 0) {
                    gesture = 3; // Scissors
                    ESP_LOGI(TAG, "Gesture set to Scissors");
                } else{
                    gesture = 0;
                }
...
void app_main(void)
{
    ...
    xTaskCreate(task_gestures, "task_gestures", 4096, NULL, 4, NULL);
}